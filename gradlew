...
if $cygwin ; then
    ...
    # Now step through the command line; replace paths with Windows paths.
    I=0
    for arg in "$@" ; do
        CHECK=`echo "$arg"|egrep -c "$OURCYGPATTERN" -`
        CHECK2=`echo "$arg"|egrep -c "^-扩展" -`
        if [ "$CHECK" -ne 0 -a "$CHECK2" -eq 0 ] ; then
            eval `echo "arguments[$I] = '"+$(cygpath --path --mixed "$arg")+"'"`
        else
            eval "arguments[$I] = '$arg'"
        fi
        I=`expr $I + 1`
    done
    set -- "${arguments[@]}"
...
# Escape WS-containing paths from the splash screen
